...
from models.DecisionTree import RegressionTree
...
def main():
    ...
    data = pd.read_csv('datasets/Temperature_Link_Oping.txt', sep="\t")

    time = np.atleast_2d(data["timestamp"].as_matrix()).T
    temp = np.atleast_2d(data["temperature"].as_matrix()).T
